...
//		GameObject obj		= NgObject.CreateGameObject(targetObject, prefab);
    ...
    //transform以下のMissingScriptを取り除く
    public static GameObject RemoveMissingScript (Transform transform)
    {
        ...
        if (hasMissing)
        {
            ...
            VerifyObject(transform.gameObject, newObj);
    ...
    //自分自身をMissingScriptなしで再生成する
    //srcObj : 元オブジェクト
    //newObj : 新オブジェクト
    protected static GameObject VerifyObject (GameObject srcObj, GameObject newObj)
    {
        Component[] objects = srcObj.GetComponents<Component>();
        foreach (Component obj in objects)
        {
            if (obj != null)
            {
                System.Type type = obj.GetType();
                Component tmp = newObj.GetComponent(type);
                if (tmp != null)
                {
                    EditorUtility.CopySerialized(obj, tmp);
                    if (obj is Transform)
                    {
                        Transform obj2 = obj as Transform;
                        for (int i = 0; i < obj2.childCount; i++)
                        {
                            obj2.GetChild(i).parent = (Transform)tmp;
                        }
                    }
                }
                else
                {
                    Component com = newObj.AddComponent(type);
                    EditorUtility.CopySerialized(obj, com);
                }
            }
        }
        newObj.transform.name = srcObj.transform.name + "(Verified)";
        newObj.name = srcObj.name + "(Verified)";
        Debug.Log("Verified.");
        return newObj;
    }
...
#if __DRECOM_
        m_fOldCameraSize = NcUtility.Camera.fieldOfView;
        NcUtility.Camera.fieldOfView = m_fOldCameraSize / captureZoomRate;
#else
...
// 2016.10.07 長坂 メインカメラ固定になってうため、変更できるように修正.
#if __DRECOM_
        NcUtility.Camera.fieldOfView = m_fOldCameraSize;
#else
...
// 2016.10.07 長坂 メインカメラ固定になってうため、変更できるように修正.
        ...
        {
            if (NcUtility.Camera.orthographic)
            {
                m_fOldCameraSize = NcUtility.Camera.orthographicSize;
                NcUtility.Camera.orthographicSize = m_fOldCameraSize / captureZoomRate;
            }
            else
            {
                m_fOldCameraSize = NcUtility.Camera.fieldOfView;
...
// 		Debug.Log("File.WriteAllBytes -------------------------------");
    ...
    #if __DRECOM_
        if (NcUtility.Camera.orthographic)
            NcUtility.Camera.orthographicSize = m_fOldCameraSize;
        else NcUtility.Camera.fieldOfView = m_fOldCameraSize;
    #else
        if (Camera.main.orthographic)
