...
// 				EditorPrefs.SetInt("FxmSpritePopup.m_nShowGameObjectOptionIndex", m_nShowGameObjectOptionIndex);
        ...
        lineRect = FXMakerLayout.GetInnerVerticalRect(baseRect, nLineCount, nIncLine++, 1);
        FXMakerLayout.m_bDevelopState = FXMakerLayout.GUIToggle (FXMakerLayout.GetInnerHorizontalRect (lineRect, 20, 0, 40), FXMakerLayout.m_bDevelopState, new GUIContent ("Development State"), true);
...
//		return FXMakertip.GetGUIContentNoTooltip(text);
	...
	public string EndCapture(Texture2D[] SpriteTextures)
	{
        ...
        Debug.Log("Export Path : " + pathTexture);
		...
		if (m_bCreatePrefab)
		{
            ...
            curveCom.m_fDurationFrame = (int)(spriteCom.GetDurationTime() / NcFrameHelper.FixedSecondPerFrame);
