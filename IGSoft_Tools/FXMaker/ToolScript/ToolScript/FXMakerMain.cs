...
// 			NgAssembly.GetPropertis(FXMakerHierarchy.inst.GetSelectedGameObject().renderer);
		...
		if (Input.GetKey(KeyCode.Z) && (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.LeftCommand) ))
			m_keyLastInput = FXMakerLayout.GetVaildInputKey(KeyCode.LeftControl , Input.GetKeyDown(KeyCode.LeftControl));
...
//		FXMakerHierarchy.inst.ChangeBoundsBoxWireframe(indexCom.m_OriginalTrans.gameObject, null, bRoot, true);
	...
	public void CreateCurrentInstanceEffect(bool bRunAction)
	{
		NgUtil.LogDevelop("CreateCurrentInstanceEffect() - bRunAction - " + bRunAction);
 		...
 		bool bTrue = CreateCurrentInstanceEffect((GetFXMakerHierarchy() != null) ?
        GetFXMakerHierarchy().GetShowGameObject() : m_OriginalEffectObject);
		if (bTrue && bRunAction)
