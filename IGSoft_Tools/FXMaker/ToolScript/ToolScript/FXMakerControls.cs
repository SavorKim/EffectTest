...
//		m_fStartPosition	= fStartAdjustRate;
    ...
    private NcController GetNcController(){
        //GameObject obj = FXMakerMain.inst.GetInstanceEffectObject();
        GameObject obj = FXMakerMain.inst.GetOriginalEffectObject();
        if(obj != null){
            return obj.GetComponent<NcController>();
        }
        return null;
    }
...
// 			TransSpeed = 1;
	...
	protected void RunActionControl(int nPlayIndex, int nTransIndex)
	{
		...
		// ﾁ､ﾁ?ｵﾈ ｽﾃｰ霏ﾏ ｰ豼?ｿ?ﾁ?ﾀﾌｵｵｷﾏ ﾇﾘｵﾎﾀﾚ...
		ResumeTimeScale();
		...
		switch (nPlayIndex)
		{
			...
			case m_nRepeatIndex+4: {
					// repeatｼｱﾅﾃﾀﾌｸ?. ﾀﾌｵｿｰ?ｷﾃ ｿﾉｼﾇﾀｻ ｲ?ｴﾙ.
...
// 2016.10.07 長坂 メインカメラ固定になってうため、変更できるように修正.
		...
		// ﾀﾌｵｿﾃｳｸｮﾁﾟﾀﾌｸ?.. ﾀﾚｵｿｹﾝｺｹｵﾇｹﾇｷﾎ ｱ簔ｸrepeatｿﾉｼﾇﾀｺ ｲ?ｴﾙ.
		if (0 < nTransIndex)
...
//		Debug.Log("CreateInstanceEffect");
	...
	public void SetTimeScale(float timeScale)
	{
		if (m_fTimeScale != timeScale || m_fTimeScale != Time.timeScale)
		{
			...
			if (0.01f <= m_fTimeScale)	// ｴﾙｽﾃ ｽﾃﾀﾛﾇﾟﾀｻｶｧ ｸﾘﾃ睇?ｻ?ﾀｸｷﾎ ﾂ?ｰ｢ｾﾈﾇﾏｵｵｷﾏ..
