...
public class FXMakerHierarchy : MonoBehaviour
{
	...
	public			enum OBJECT_TYPE	{ OBJECT_GAMEOBJECT, OBJECT_TRANSFORM,
        OBJECT_EASYEFFECT, OBJECT_UNITYENGINE, OBJECT_ANICLIP, OBJECT_MATERIAL, OBJECT_OTHER };
    ...
    protected		GameObject				m_DeletedComponent;
    protected		GameObject				m_DeletedComponentParent;
	...
	void Update()
	{
		...
		// ShotKey - Delete
		if (FXMakerMain.inst.GetFocusInputKey(
                FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.EFFECT_HIERARCHY)) == KeyCode.Delete ||
        FXMakerMain.inst.GetFocusInputKey(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.NONE)) == KeyCode.Delete)
		{
			if (m_ActiveComponent != null && (m_ActiveComponent is Transform) == false &&
            (m_ActiveComponent is GameObject || m_ActiveComponent is Component))
			{
        ...
        // ShotKey - Undo
        if (FXMakerMain.inst.GetFocusInputKey(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.NONE)) == KeyCode.LeftControl ||
            FXMakerMain.inst.GetFocusInputKey(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.EFFECT_HIERARCHY)) == KeyCode.LeftControl || 
            FXMakerMain.inst.GetFocusInputKey(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.NONE)) == KeyCode.LeftCommand ||
            FXMakerMain.inst.GetFocusInputKey(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.EFFECT_HIERARCHY)) == KeyCode.LeftCommand)
        {
            UndoDeleteHierarchyObject();
        }
...
// 		Debug.Log(x);
	...
	public void DeleteHierarchyObject(Transform baseTrans, Object delObj, int nSelectedIndex)
	{
		// enable save
		FXMakerEffect.inst.SetChangePrefab();
        if(m_DeletedComponent != null){
            GameObject.DestroyImmediate(m_DeletedComponent);
            m_DeletedComponent = null;
        }
        if(m_DeletedComponentParent != null){
            m_DeletedComponentParent = null;
        }
        m_DeletedComponent = (GameObject)GameObject.Instantiate(delObj);
        GameObject delObj_ = (GameObject)delObj;
        m_DeletedComponentParent = delObj_.transform.parent.gameObject;
    ...
    public void UndoDeleteHierarchyObject()
    {
        if(m_DeletedComponentParent == null || m_DeletedComponent == null){
            return;
        }
        AddGameObject(m_DeletedComponentParent, m_DeletedComponent);
        m_DeletedComponent = null;
        m_DeletedComponentParent = null;
        FXMakerMain.inst.CreateCurrentInstanceEffect(true);
        Debug.Log("restored");
...
//		FxmPopupManager.inst.SetScriptTooltip("");
		...
		if (m_CurrentEffectObject != null)
		{
			...
			if (m_bUpdateActiveComponent == false)	// ｰﾋｻ?ｿﾉｼﾇｿ｡ ﾀﾇﾇﾘ ｾ?ｾ??
...
//					((Behaviour)selObj).enabled = ret;
	...
	{
		Object tarObj;
		if (m_DragObjectTrans == currentTrans)	// ｼ?ｼｭ ｹﾙｲﾞ
		{
...
//			GUI.Box(boxRect, new GUIContent(caption, FXMakerMain.HoverCommand_Hierarchy(selObj)), style);
	...
	public GameObject AddGameObject(GameObject parent, GameObject addObj)
	{
        ...
        Renderer rens = newObj.GetComponentInChildren<Renderer>();
        if (rens != null) {
            NcSortingLayer sortingLayer = rens.gameObject.GetComponent<NcSortingLayer>();
            if(sortingLayer != null){
                Object.DestroyImmediate(sortingLayer);
            }

            NcSortingLayerMobileFixed sortingLayerFixed = rens.gameObject.GetComponent<NcSortingLayerMobileFixed>();
            if(sortingLayerFixed == null){
                sortingLayerFixed = rens.gameObject.AddComponent<NcSortingLayerMobileFixed>();
            }
        }
