...
public class FxmTestControls : MonoBehaviour
{
	...
	bool IsAliveAnimation()
	{
		...
		foreach (Transform trans in tranComs)
		{
			...
			{
				bParticle = 0;
				if (NgObject.IsActive(trans.gameObject) && 
					((trans.GetComponent<ParticleSystem>().emission.enabled && trans.GetComponent<ParticleSystem>().IsAlive()) || 0 < trans.GetComponent<ParticleSystem>().particleCount))
					bParticle = 1;
			}
