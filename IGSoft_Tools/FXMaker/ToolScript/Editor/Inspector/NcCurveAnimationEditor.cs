...
public class NcCurveAnimationEditor : FXMakerEditor
{
    ...
    private int _beforeDelayFrame = 0;
    private float _beforeDelayTime = 0;
    private int _beforeDurationFrame = 0;
    private float _beforeDurationTime = 0;
...
// 			FxmInfoIndexing.FindInstanceIndexing(m_Sel.transform, false).GetComponent<NcCurveAnimation>().ResumeAnimation();
		...
		EditorGUI.BeginChangeCheck();
		{
            ...
            m_Sel.m_fDelayFrame = EditorGUILayout.IntField(GetHelpContent("m_fDelayFrame")			, m_Sel.m_fDelayFrame);
            m_Sel.m_fDurationFrame	= EditorGUILayout.IntField(GetHelpContent("m_fDurationFrame")		, m_Sel.m_fDurationFrame);
            SetMinValue(ref m_Sel.m_fDelayFrame, 0);
            SetMinValue(ref m_Sel.m_fDurationFrame, 1);
				...
				EditorGUI.BeginChangeCheck();
				{
					rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nLineHeight));
					{
						...
						if (curveInfoList[n].m_ApplyType == NcCurveAnimation.NcInfoCurve.APPLY_TYPE.TEXTUREUV ||
							curveInfoList[n].m_ApplyType == NcCurveAnimation.NcInfoCurve.APPLY_TYPE.TEXTUREUV_OFFSET)
						{
