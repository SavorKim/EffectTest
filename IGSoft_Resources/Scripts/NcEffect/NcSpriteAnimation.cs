...
{
	// Attribute ------------------------------------------------------------------------
    public		enum			TEXTURE_TYPE				{TileTexture, TrimTexture, SpriteFactory, MeshUv, MeshUvMirror};
    ...
    [SerializeField]
    private float m_fFpsBase = 10;

    public float m_fFps
    {
        get
        {
            return m_fFpsBase * base.EffectTimeScale_;
        }

        set
        {            
            m_fFpsBase = Mathf.Max(value, 0.1f);
        }
    }

    public float FpsBase
    {
        get
        {
            return m_fFpsBase;
...
// 	public		float			m_fBlueAlphaWeight			= 1;
	...
	{
        if (m_TextureType == NcSpriteAnimation.TEXTURE_TYPE.TileTexture || m_TextureType == TEXTURE_TYPE.MeshUv || m_TextureType == NcSpriteAnimation.TEXTURE_TYPE.MeshUvMirror)
	...
	void SetIndex(int nSeqIndex)
	{
		if (m_Renderer != null)
		{
            ...
            else if (m_TextureType == TEXTURE_TYPE.MeshUvMirror)
            {
                ...
                m_MeshFilter.mesh.SetUVs(1, new List<Vector4>(){uv,uv,uv,uv});
            }
