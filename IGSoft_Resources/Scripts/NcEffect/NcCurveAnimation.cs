...
public class NcCurveAnimation : NcEffectAniBehaviour
{
	...
	public class NcInfoCurve
	{
		...
		public		enum APPLY_TYPE						{ NONE, POSITION, ROTATION, SCALE, MATERIAL_COLOR, TEXTUREUV, MESH_COLOR, TEXTUREUV_OFFSET };
		public		static string[]	m_TypeName			= { "None", "Position", "Rotation", "Scale", "MaterialColor", "TextureUV", "MeshColor", "TextureUV Offset" };
...
// 			target.m_ToValue		= m_ToValue;
		...
		public int GetValueCount()
		{
			switch (m_ApplyType)
			{
				...
				case NcInfoCurve.APPLY_TYPE.TEXTUREUV_OFFSET: return 2;
		...
		public string GetValueName(int nIndex)
		{
			...
			switch (m_ApplyType)
			{
				...
				case NcInfoCurve.APPLY_TYPE.TEXTUREUV_OFFSET: valueNames	= new string[4] { "X", "Y", "", "" };			break;
				case NcInfoCurve.APPLY_TYPE.NONE:
		...
		public void SetDefaultValueScale()
		{
			switch (m_ApplyType)
			{
				...
				case NcInfoCurve.APPLY_TYPE.TEXTUREUV_OFFSET: m_fValueScale	= 1;	break;
...
// 				range.xMax = Mathf.Max(range.yMax, m_AniCurve[n].time);
    ...
    public 		int 				m_fDelayFrame  = 0;
    public 		int 				m_fDurationFrame  = 50;
    public		float				m_fDelayTime = 0;
    public		float				m_fDurationTime  = 0.7f;
	...
	public override string CheckProperty()
	{
		...
		foreach (NcInfoCurve curveInfo in m_CurveInfoList)
		{
			...
			if (curveInfo.m_ApplyType == NcInfoCurve.APPLY_TYPE.TEXTUREUV_OFFSET)
				if (GetComponent("NcUvAnimation") == null)
					return "SCRIPT_NEED_NCUVANIMATION";
...
// 	}
	...
	{
#if UNITY_EDITOR
        m_fDurationFrame = (int)( m_fDurationTime / NcFrameHelper.FixedSecondPerFrame);
        m_fDelayFrame = (int)( m_fDelayTime / NcFrameHelper.FixedSecondPerFrame);
#endif
		ResetPosition();
...
//										AddRuntimeMaterial(ren.material);
				...
				case NcInfoCurve.APPLY_TYPE.TEXTUREUV_OFFSET:
					{
						if (m_NcUvAnimation == null) {
							m_NcUvAnimation	= GetComponent<NcUvAnimation> ();
						}
						...
						{
							if (m_bSavedOriginalValue == false) {
								curveInfo.m_OriginalValue = new Vector4 (m_NcUvAnimation.m_fOffsetX, m_NcUvAnimation.m_fOffsetY, 0, 0);
							}
							else {
								m_NcUvAnimation.m_fOffsetX	= curveInfo.m_OriginalValue.x;
								m_NcUvAnimation.m_fOffsetY	= curveInfo.m_OriginalValue.y;
							}
						}
						curveInfo.m_BeforeValue		= curveInfo.m_OriginalValue;
						break;
