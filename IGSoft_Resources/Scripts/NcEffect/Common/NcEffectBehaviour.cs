...
public class NcEffectBehaviour : MonoBehaviour
{
    ...
    protected	GameObject		m_OnDestroyGameObject	= null;
    public		string			m_OnDestroyFunction		= "OnDestroy";

    private NcController _controller = null;

    protected NcController NcController
    {
        get
        {
            if (_controller == null)
            {
                _controller = GetComponentsInParent<NcController>(true).First();
            }
            return _controller;
        }
    }

    protected float EffectTimeScale_
    {
        get
        {
            return NcController.EffectTimeScale;
        }
    }
    
    // Property -------------------------------------------------------------------------

    public void SetCallBackOnDestroy(GameObject callBackGameObj, string nameFunction)
    {
        m_OnDestroyGameObject	= callBackGameObj;
        m_OnDestroyFunction		= nameFunction;
    }
	...
	public static GameObject GetRootInstanceEffect()
	{
		...
		if (m_RootInstance == null)
		{
            ...
            if (m_RootInstance == null)
            {
                ...
                m_RootInstance.AddComponent<NcController>();
            }
		}
...
*/
	...
	{
        return (!m_bShuttingDown && SceneManager.GetSceneByName("sceneFXMaker").isLoaded);
        //return (NcSafeTool.IsSafe()); //NcSafeToolは使われていない
	}
...
// 				com.SendMessage("Update", SendMessageOptions.DontRequireReceiver);
	...
	{
        if (m_OnDestroyGameObject != null)
            m_OnDestroyGameObject.SendMessage(m_OnDestroyFunction, this, SendMessageOptions.DontRequireReceiver);
		// RuntimeMaterials
		if (m_RuntimeMaterials != null)
		{
