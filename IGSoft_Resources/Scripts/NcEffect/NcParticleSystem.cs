...
// 			SetEnableParticle(true);
	...
	void Update()
	{
		...
		if (m_fEmitStartTime == 0)
		{
			...
			if (0 < m_fStartDelayTime)
			{
                if (m_fStartTime + (m_fStartDelayTime / base.EffectTimeScale_) <= NcController.GetEngineTime())
				{
                    m_fEmitStartTime	= NcController.GetEngineTime();
                    m_fDurationStartTime= NcController.GetEngineTime();
					SetEnableParticle(true);
				}
				return;
			} else {
		...
		if (m_bBurst)
		{
            if (m_fDurationStartTime <= NcController.GetEngineTime())
			{
				if ((m_nBurstRepeatCount == 0 || m_nCreateCount < m_nBurstRepeatCount))		// check repeat count
				{
//					SetEnableParticle(true);
                    m_fDurationStartTime = (m_fBurstRepeatTime / base.EffectTimeScale_) + NcController.GetEngineTime();
					m_nCreateCount++;
					if (IsShuriken())
...
//				SetEnableParticle(false);
			...
			if (m_bSleep)
			{
                if (m_fEmitStartTime + (m_fEmitTime / base.EffectTimeScale_)+ (m_fSleepTime / base.EffectTimeScale_) < NcController.GetEngineTime())
				{
					SetEnableParticle(true);
                    m_fEmitStartTime = NcController.GetEngineTime();
					m_bSleep = false;
				...
				// Emit End
                if (0 < m_fEmitTime && m_fEmitStartTime + (m_fEmitTime / base.EffectTimeScale_) < NcController.GetEngineTime())
				{
					m_nCreateCount++;
					SetEnableParticle(false);
					if (0 < m_fSleepTime)
