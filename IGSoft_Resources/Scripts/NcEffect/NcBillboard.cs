...
// 2016.10.07 éLÛà ¥á¥¤¥ó¥«¥á¥é¹Ì¶¨¤Ë¤Ê¤Ã¤Æ¤¦¤¿¤á¡¢‰ä¸ü¤Ç¤­¤ë¤è¤¦¤ËÐÞÕý.
        ...
        // Ä«¸Þ¶ó ¾÷º¤ÅÍ¸¦ ¹«½ÃÇÏ°í ¿ÀÁ§ÀÇ ¾÷º¤ÅÍ¸¦ À¯ÁöÇÑ´Ù
        if (m_bFixedObjectUp)
//      vecUp       = m_qOiginal * Vector3.up;
            vecUp       = transform.up;
        else vecUp      = NcUtility.Camera.transform.rotation * Vector3.up;

        if (m_bCameraLookAt)
            transform.LookAt(NcUtility.Camera.transform, vecUp);
        else transform.LookAt(transform.position + NcUtility.Camera.transform.rotation * Vector3.back, vecUp);
#else
        if (Camera.main == null)
            return;
