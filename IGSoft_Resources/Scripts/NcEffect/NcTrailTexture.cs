...
// 			return "SCRIPT_WARRING_DUPLICATE";
	...
	void OnDisable()
	{
		if (m_TrialObject != null)
            NcAutoDestruct.CreateAutoDestruct(m_TrialObject, 0, (m_fLifeTime / base.EffectTimeScale_) / 2.0f, true, true);
	}
...
// 		m_TrialObject.AddComponent<Nc>();
	...
	void Update()
	{
		...
		{
            if (m_fStartTime + (m_fEmitTime / base.EffectTimeScale_) < NcController.GetEngineTime())
			{
				if (m_bSmoothHide)
                    m_fStopTime = NcController.GetEngineTime();
				else m_bEmit = false;
			}
		}
        if (0 < m_fStopTime && (m_fLifeTime / base.EffectTimeScale_) < (NcController.GetEngineTime() - m_fStopTime))
			m_bEmit = false;
...
// 		if (!Camera.main) return;
		...
		if (m_bEmit)
		{
			if (theDistance > m_fMinVertexDistance)
			{
				...
				if (make)
				{
					...
					if (0 < m_fStopTime)
					{
                        p.timeCreated	= NcController.GetEngineTime() - (NcController.GetEngineTime() - m_fStopTime);
					} else {
                        p.timeCreated	= NcController.GetEngineTime();
					}
					m_Points.Add(p);
					m_LastPosition = transform.position;
	...
	//						IEnumerable<Vector3> smoothBase = NcInterpolate.NewBezier(NcInterpolate.Ease(NcInterpolate.EaseType.Linear), basePoints, m_nSubdivisions);
		...
		foreach (Point p in m_Points)
		{
            ...
            if (NcController.GetEngineTime() - p.timeCreated > (m_fLifeTime / base.EffectTimeScale_))
			{
				remove.Add(p);
			}
		}
		...
		if (pointsToUse.Count > 1)
		{
			...
			for (int n = 0; n < pointsToUse.Count; ++n)
			{
                ...
                float time = (NcController.GetEngineTime() - p.timeCreated) / (m_fLifeTime / base.EffectTimeScale_);
